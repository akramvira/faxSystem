...
var API = function () {
     ...
     * @example var pd = jq('.selector').persianDatepicker();
    ...
    _createClass(API, [{
        ...
        set: function set(inputOptions) {
            var opt = jq.extend(true, this.model.options, inputOptions);
            this.model.view.destroy();
...
(function ($) {
    ...
    jq.fn.persianDatepicker = jq.fn.pDatepicker = function (options) {
        var args = Array.prototype.slice.call(arguments),
            output = null,
            self = this;
        if (!this) {
            jq.error('Invalid selector');
        }
        jq(this).each(function () {
            // encapsulation Args
            var emptyArr = [],
                tempArg = args.concat(emptyArr),
                dp = jq(this).data('datepicker'),
                funcName = null;
        ...
        jq(this).data('datepicker', self.pDatePicker);
...
var Input = function () {
    ...
    _createClass(Input, [{
        ...
        value: function addInitialClass() {
            jq(this.elem).addClass('pwt-datepicker-input-element');
        }
    }, {
        key: 'parseInput',
        ...
        value: function observe() {
            ...
            jq(that.elem).bind('paste', function (e) {
            ...
            jq(document).keydown(function (e) {
                if (jq.inArray(e.keyCode, ctrlKey) > 0) ctrlDown = true;
            }).keyup(function (e) {
                if (jq.inArray(e.keyCode, ctrlKey) > 0) ctrlDown = false;
            });

            jq(that.elem).bind('keyup', function (e) {
                var $self = jq(this);
                var trueKey = false;
                if (e.keyCode === 8 || e.keyCode < 105 && e.keyCode > 96 || e.keyCode < 58 && e.keyCode > 47 || ctrlDown && (e.keyCode == vKey || jq.inArray(e.keyCode, ctrlKey) > 0)) {
                    trueKey = true;
                }
                if (trueKey) {
            ...
            jq(that.elem).on('keydown', function () {
                clearTimeout(typingTimer);
            });
            ...
            //         let newDate = new Date(jq(this).val());
        ...
        value: function _attachInputElementEvents() {
            ...
            var closePickerHandler = function closePickerHandler(e) {
                if (!jq(e.target).is(that.elem) && !jq(e.target).is(that.model.view.$container) && jq(e.target).closest('#' + that.model.view.$container.attr('id')).length == 0 && !jq(e.target).is(jq(that.elem).children())) {
                    that.model.api.hide();
                    jq('body').unbind('click', closePickerHandler);
                }
            };

            jq(this.elem).on('focus click', Helper.debounce(function (evt) {
                that.model.api.show();
                if (that.model.state.ui.isInline === false) {
                    jq('body').unbind('click', closePickerHandler).bind('click', closePickerHandler);
                }
                if (Helper.isMobile) {
                    jq(this).blur();
                }
            ...
            jq(this.elem).on('keydown', Helper.debounce(function (evt) {
                if (evt.which === 9) {
                    that.model.api.hide();
                    return false;
                }
        ...
        value: function getInputSize() {
            return {
                width: jq(this.elem).outerWidth(),
                height: jq(this.elem).outerHeight()
            };
        }
        ...
        value: function _updateAltField(unix) {
            ...
            jq(this.model.options.altField).val(value);
        }
        ...
        value: function _updateInputField(unix) {
            ...
            if (jq(this.elem).val() != value) {
                jq(this.elem).val(value);
            }
        }
    ...
    _createClass(Navigator, [{
        ...
        value: function liveAttach() {
            ...
            if (this.model.options.navigator.scroll.enabled) {
                var that = this;
                var gridPlot = jq('#' + that.model.view.id + ' .datepicker-grid-view')[0];
                ...
                if (this.model.options.timePicker.enabled) {
                    jq('#' + that.model.view.id + ' .time-segment').each(function () {
        ...
        value: function _attachEvents() {
            ...
            if (this.model.options.navigator.enabled) {
                 ...
                 */
                jq(document).on('click', '#' + that.model.view.id + ' .pwt-btn', function () {
                    if (jq(this).is('.pwt-btn-next')) {
                        that.model.state.navigate('next');
                        that.model.view.render();
                        that.model.options.navigator.onNext(that.model);
                    } else if (jq(this).is('.pwt-btn-switch')) {
                        that.model.state.switchViewMode();
                        that.model.view.render();
                        that.model.options.navigator.onSwitch(that.model);
                    } else if (jq(this).is('.pwt-btn-prev')) {
                        that.model.state.navigate('prev');
                        that.model.view.render();
            ...
            if (this.model.options.timePicker.enabled) {
                 ...
                 */
                jq(document).on('click', '#' + that.model.view.id + ' .up-btn', function () {
                    var timekey = jq(this).data('time-key');
                    that.timeUp(timekey);
                 ...
                 */
                jq(document).on('click', '#' + that.model.view.id + ' .down-btn', function () {
                    var timekey = jq(this).data('time-key');
            ...
            if (this.model.options.dayPicker.enabled) {
                 ...
                 */
                jq(document).on('click', '#' + that.model.view.id + ' .datepicker-day-view td:not(.disabled)', function () {
                    var thisUnix = jq(this).data('unix'),
                        mustRender = void 0;
            ...
            if (this.model.options.monthPicker.enabled) {
                 ...
                 */
                jq(document).on('click', '#' + that.model.view.id + ' .datepicker-month-view .month-item:not(.month-item-disable)', function () {
            ...
            if (this.model.options.yearPicker.enabled) {
                 ...
                 */
                jq(document).on('click', '#' + that.model.view.id + ' .datepicker-year-view .year-item:not(.year-item-disable)', function () {
...
var Options = function () {
    ...
    function Options(options, model) {
        ...
        this.model = model;
        return this._compatibility(jq.extend(true, this, Config, options));
    }
...
var Toolbox = function () {
    ...
    _createClass(Toolbox, [{
        ...
        value: function _attachEvents() {
            var that = this;
            jq(document).on('click', '#' + that.model.view.id + ' .pwt-btn-today', function () {
            ...
            jq(document).on('click', '#' + that.model.view.id + ' .pwt-btn-calendar', function () {
            ...
            jq(document).on('click', '#' + that.model.view.id + ' .pwt-btn-submit', function () {
                that.model.view.hide();
...
var View = function () {
    ...
    function View(model) {
        ...
        if (this.model.state.ui.isInline) {
            this.$container = jq('<div  id="' + this.id + '" class="datepicker-container-inline"></div>').appendTo(that.model.inputElement);
        } else {
            this.$container = jq('<div  id="' + this.id + '" class="datepicker-container"></div>').appendTo('body');
            this.hide();
    ...
    _createClass(View, [{
        ...
        value: function markSelectedDay() {
            ...
            this.$container.find('.table-days td').each(function () {
                if (jq(this).data('date') == [selected.year, selected.month, selected.date].join(',')) {
        ...
        value: function markToday() {
            ...
            this.$container.find('.table-days td').each(function () {
                if (jq(this).data('date') == [today.year(), today.month(), today.date()].join(',')) {
                    jq(this).addClass('today');
                } else {
                    jq(this).removeClass('today');
                }
            });
        }
        ...
        value: function render(data) {
            ...
            this.rendered = jq(Mustache.render(this.model.options.template, this.getViewModel(data)));
