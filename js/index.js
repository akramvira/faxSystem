    ...
    callback: function(data) {
      ...
      for (var i = 0; i < sendingFiles.length; i++) {
        sendingFileList += `
                    <h5 class="card-title file-title">- ${
                      sendingFiles[i]["namefile"]
                    }</h5>
                    <p class="card-text">
                        <span class="file-name p-1">از: ${
                          sendingFiles[i]["from"]
                        }</span>
                        <span class="file-from p-1">به:${
                          sendingFiles[i]["to"]
                        }</span>
                        <span class="file-from p-1">ntries:${
                          sendingFiles[i]["ntries"]
                        }</span>
                        <span class="file-from p-1">state:${
                          sendingFiles[i]["state"]
                        }</span>
                        <span class="file-from p-1">فایل:${
                          sendingFiles[i]["file"]
                        }</span>
      ...
      var myPieChart = new Chart(ctxP, {
        ...
        data: {
          ...
          datasets: [
            {
              data: [data["archive"], data["send"], data["receive"]],
